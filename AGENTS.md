...
The host machine's Node version is not reliable for this project's toolchain
(`@kinvolk/headlamp-plugin`'s CLI and rollup's native bindings both break on
newer Node majors). A pinned Node 22 dev container lives in `.devcontainer/`
and is the reproducible way to run any of the scripts below:

```bash
./scripts/dev-container.sh install   # npm install (first time, or after package.json changes)
./scripts/dev-container.sh build     # npm run build
./scripts/dev-container.sh test      # npm run test
./scripts/dev-container.sh sh        # shell inside the container
```

`node_modules` lives in a named Docker volume (not baked into the image and
not the host's `node_modules/`), so repeat runs are fast without giving up
reproducibility — `install` always runs against whatever `package.json`/
`package-lock.json` are on disk. The same `.devcontainer/` also works with
VS Code's Dev Containers extension / GitHub Codespaces.

Note: `headlamp-plugin build`'s "Successfully copied extra dist files" step
does not copy `package.json` into `dist/` — copy it manually (or as part of
your deploy step) before placing a build in a Headlamp plugins directory;
the backend plugin loader logs a warning and falls back to the folder name
without it.
